  ...
  function getClientScripts(config, pluginsMarkup) {
    ...
    if (localConfig.client.app.useVersionedAssets) {
      _.each(clientResource, function(value, key) {
        if (value) {
          clientResource[key] = value.replace(localConfig.client.app.assetPath, localConfig.client.app.versionedAssetPath);
        }
      });

      var scriptReplaceRegex = new RegExp('src="' + localConfig.client.app.assetPath + '([^"]+)"', 'g');

      pluginsMarkup = pluginsMarkup.replace(scriptReplaceRegex, 'src="' + localConfig.client.app.versionedAssetPath + '$1"');
    }
