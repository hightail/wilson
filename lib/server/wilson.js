...
module.exports = function(app, appWilsonConfigJson, dependencies) {
  ...
  container.register('SassService', require('./services/SassService'));
  ...
  container.register('routes/css', require('./routes/css'));
  ...
  //PRERENDER
  var prerenderServiceUrl = wilsonConfig.server.prerender.prerenderServiceUrl;
  if (!_.isEmpty(prerenderServiceUrl)) {
    app.use(require('prerender-node')).set('prerenderServiceUrl', prerenderServiceUrl);//.set('prerenderToken', 'WILSON_PRERENDER_TOKEN'));
  }
  ...
  //CSS
  router.get('/:version?/wilson.css', container.get('routes/css'));
  router.get('/:version?/wilson.min.css', container.get('routes/css'));
