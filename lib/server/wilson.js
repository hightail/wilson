...
module.exports = function(app, appWilsonConfigJson, dependencies) {
  ...
  if (process.env.PRERENDER_SERVICE_URL && !_.isEmpty(process.env.PRERENDER_SERVICE_URL)) {
    ...
    if (wilsonConfig.client.prerender.whitelist) { prerenderNode.whitelisted(wilsonConfig.client.prerender.whitelist); }
    if (wilsonConfig.client.prerender.blacklist) { prerenderNode.blacklisted(wilsonConfig.client.prerender.blacklist); }
    
    app.use(prerenderNode);//.set('prerenderToken', 'WILSON_PRERENDER_TOKEN'));
  }
