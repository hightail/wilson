...
module.exports = function(app, appWilsonConfigJson, dependencies) {
  ...
  container.register('middleware/debug-mode', require('./middleware/debug-mode'));
  ...
  var wilsonDebugModeMiddleware = container.get('middleware/debug-mode');
  ...
  if (process.env.PRERENDER_SERVICE_URL && !_.isEmpty(process.env.PRERENDER_SERVICE_URL)) {
    var prender = require('prerender-node');
    
    // Apply Pre-render Config
    if (wilsonConfig.client.prerender.whitelist) { prerender.whitelisted(wilsonConfig.client.prerender.whitelist); }
    if (wilsonConfig.client.prerender.blacklist) { prerender.blacklisted(wilsonConfig.client.prerender.blacklist); }
    
    app.use(prerender);//.set('prerenderToken', 'WILSON_PRERENDER_TOKEN'));
  }
