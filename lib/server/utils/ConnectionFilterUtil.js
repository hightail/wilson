...
module.exports = function(wilsonConfig) {
  ...
  var getConnectionFilterArray = function (tags, supportMultiValues) {
    var filters     = [];
    var delimiter   = '+';

    _.each(tags, function (filterValue, filterName) {
      if (supportMultiValues && filterValue.indexOf(delimiter) !== -1) {
        _.each(filterValue.split(delimiter), function(subval) {
          filters.push({ name: filterName, value: subval, priority: getFilterPriority(filterName) });
        });
      } else {
  ...
  var getFilterString = function (filters) {
    ...
    _(filters).sortBy('name').each(function (filter) {
      filterArray.push(_.sprintf('%s=%s', filter.name, filter.value));
    });
