...
module.exports = function(wilsonConfig) {
  ...
  var findRevisionedFiles = function (directoryPath, fileNameList) {
    ...
    return fs.readdir(path.join(localPathPrefix, directoryPath)).then(function (err, directoryContents) {
      console.log('err', err);
      ...
      if (revedFileList.length === fileNameList.length) {
        return revedFileList;
      } else {
        return _.sprintf('Some revisioned files were not found (Found %d of %d)', revedFileList.length, fileNameList.length);
      }
