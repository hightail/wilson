...
module.exports = function(wilsonConfig) {
  ...
  var findRevisionedFiles = function (directoryPath, fileNameList) {
    ...
    fs.readdir(path.join(localPathPrefix, directoryPath)).then(function (err, directoryContents) {
      ...
      if (revedFileList.length === fileNameList.length) {
        ...
        deferred.reject(_.sprintf('Some revisioned files were not found (Found %d of %d)', revedFileList.length, fileNameList.length));
      }
    }).fail(function (error) {
