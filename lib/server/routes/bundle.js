...
module.exports = function(wilsonConfig, BundleService, ComponentService) {
  return function(req, res) {
     ...
     * @param res
     */
    var setResponseCacheHeaders = function(res, maxAgeMs, ignoreCache) {
      if (ignoreCache) {
        ...
        res.setHeader('Cache-Control', 'public, max-age=11');
    ...
    if (!requestVersion) {
      ...
      if (requestVersion !== appVersion) {
        ...
        if (fs.existsSync(bundleFile) && !ignoreCache) {
          served = true;
          setResponseCacheHeaders(res, cacheMaxAgeMs, ignoreCache);
