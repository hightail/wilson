...
module.exports = function(wilsonConfig, logger, ComponentService) {
  ...
  function registerHandlebarsHelpers(hbsRef) {
    console.log('registerHandlebarsHelpers: register helper');
    // i18n basic helper
  ...
  function init(app) {
    ...
    console.log('i18NModule::init before i18next.init');
    i18next.use(middleware.LanguageDetector).use(backend).init(i18nextOptions);
    console.log('i18NModule::init after i18next.init');
    ...
    console.log('i18NModule::init translationNamespaces', translationNamespaces);
    ...
    i18next.loadNamespaces(translationNamespaces, function() {
      ...
      console.log('i18NModule:: loaded i18next namespaces:', translationNamespaces);
  ...
  function i18nConfigMiddleware(req, res, next) {
    ...
    // If route localization is configured, then update the routes to include the locale in the path
    if (wilsonConfig.client.app.autoLocalizeRoutes) {
      req.wilson.routes = getLocalizedRoutes(lng);
    } else {
      // If not, then just use the original routing info (clone deep to avoid changing the closured object reference)
      req.wilson.routes = _.cloneDeep(routingJson.routes);
    }
    ...
    loadResourcesForLanguage(lng).then(
        function(resStore) {
          ...
          console.log('i18nConfigMiddleware: print request', reqI18N);
          next();
        },
        function(error) {
  ...
  function loadResourcesForLanguage(lng) {
    return getI18nResourceStore(lng, i18next.options.ns);
  }
    ...
    if (resStore[lng]) {
      ...
      asyncUtil.eachLimit(namespaces, 40, function (namespace, callback) {
        ...
        if (missingNameSpaces.length > 0) {
          logger.warn('Resource bundle JSON not found for some namespaces.');
        }
  ...
  function getLocaleFromUrl(requestUrl, defaultLocale) {
    ...
    var matchedLocaleSupported  = _.includes(supportedLocales, matchedLocale);
    ...
    if (matchedLocale && !matchedLocaleSupported && (matchedLocale.indexOf('-') !== -1)) {
      ...
      matchedLngSupported = _.includes(supportedLocales, matchedLng);
    }
  ...
  function getLanguageFromUrlMiddleware(req, res, next) {
    ...
    //console.log('lng', lng);
    if(redirectUrl) {
      console.log('getLanguageFromUrlMiddleware: redirectUrl', redirectUrl);
      //If the language in the URL is something weird, redirect to make things pretty
      res.redirect(redirectUrl);
      res.end();
    } else {
      console.log('getLanguageFromUrlMiddleware: lng', lng);
      //Set language tag
