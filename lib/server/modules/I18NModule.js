var i18next       = require('i18next-node-stable'),
...
module.exports = function(wilsonConfig, logger, ComponentService) {
  ...
  function init(app) {
    ...
    console.log('i18next.handle', JSON.stringify(i18next.handle));
  }
  ...
  function i18nConfigMiddleware(req, res, next) {
    ...
    loadResourcesForLanguage(lng).then(
        function(resStore) {
          ...
          console.log('i18nConfigMiddleware: print request', reqI18N.wilson, reqI18N.lng, reqI18N.namespaces, reqI18N.resStore);
          next();
        },
        function(error) {
  ...
  function i18nextHandle() {
    console.log('i18next.handle called');
