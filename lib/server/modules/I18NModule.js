...
module.exports = function(wilsonConfig, logger, ComponentService) {
  ...
  _.merge(i18nextOptions, wilsonConfig.client.i18n);
  ...
  function registerHandlebarsHelper() {
    // i18n basic helper
    hbs.registerHelper('__', function(key, options) {
      ...
      return new hbs.SafeString(i18next.t(key, i18nOptions));
    });
  }
        ...
        if (fileExists) {
          ...
          try {
            ...
            if (resStore[lng] && resStore[lng][namespace]) {
              _.merge(resStore[lng][namespace], namespaceJson);
            } else {
  ...
  return {
    ...
    loadResourcesForLanguage: loadResourcesForLanguage
  };
}