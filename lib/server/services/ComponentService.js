...
module.exports = function(wilsonConfig, wilsonFrameworkConfig, logger, CacheService, BundleService, RevisionedFileUtil) {
  ...
  var getListOfComponentNames = function (componentPath) {
    ...
    if (fs.existsSync(componentPath)) {
      ...
      componentNames = _.filter(componentNames, function (componentName) {
        ...
        return !_.includes(ignoredFiles, componentName);
      });
    } else {
  ...
  var getServiceDependencies = function (scriptPath, mode) {
    ...
    switch (mode) {
      ...
      case 'behavior':
        controllerDependancyRegex = /.*controller:\[((['"][^'"]+['"],){0,}){1}function\(.*/g;
        directiveDependancyRegex = /.*wilson\.behavior\(['"][^'"]+['"],\[((['"][^'"]+['"],){0,}){1}function\(.*/g;
      ...
      //case services:
      default:
        // Default to service mode - support here fore several different alias regex for services
        captureGroup = '$2';
        dependencyRegexps =[
          /.*wilson\.(service|class|factory|resource|utility)\(['"][^'"]+['"],\[((['"][^'"]+['"],){0,}){1}function\(.*/g
        ];
    }
    ...
    _.each(dependencyRegexps, function (regex) {
      if (regex.test(content)) {
        var deps = content.replace(regex, captureGroup).replace(/['"]/g, '').split(',');
  ...
  var init = function (options) {
    ...
    _.each(config.routes, function (routeInfo) {
      if (_.indexOf(topComponents, routeInfo.component) === -1) {
        var msg = _.sprintf('Illegal route! All routes MUST point to page-level components. Cannot route path [%s] to component [%s]!', routeInfo.path, routeInfo.component);
        logger.error(msg);
        throw new Error(msg);
      }
    });
  ...
  var getBehaviorScripts = function (behaviorName) {
    ...
    // Otherwise, find related behavior scripts
    var scripts = glob.sync(path.join(behaviorsPath, '**', behaviorName + '.js'));

    if (!scripts.length) { throw new Error(_.sprintf('Behavior "%s" was not found in path: %s', behaviorName, behaviorsPath)); }

    // Cache scripts into the build-time cache
    buildTimeCache.behaviorScripts[behaviorName] = scripts;
  ...
  var getGuideScripts = function (guideName) {
    ...
    try {
      ...
      _.each(scriptList, function (script) {
        if (_.endsWith(script, guideName + '.js')) {
