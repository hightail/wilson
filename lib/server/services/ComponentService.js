...
module.exports = function(wilsonConfig, wilsonFrameworkConfig, logger, CacheService, BundleService, RevisionedFileUtil) {
  ...
  var getServiceNameFromFilePath = function (serviceFilePath) {
    var serviceName = '';
    if (_.includes(serviceFilePath, '.js')) {
      serviceFilePath.split('/').pop().replace('.js', '');
    } else if (_.includes(serviceFilePath, '.ts')) {
      serviceFilePath.split('/').pop().replace('.ts', '');
    }
  ...
  function findAllComponents() {
    var components        = [];
    var componentScripts  = glob.sync(path.join(componentsPath, '**', '*.js'));
    var componentTScripts = glob.sync(path.join(componentsPath, '**', '*.ts'));

    _.each(componentScripts, function(compScript) {
      components.push(getComponentFromFilePath(compScript));
    });

    _.each(componentTScripts, function(compTScript) {
      components.push(getComponentFromFilePath(compTScript));
    });
  ...
  var generateServiceLibrary = function () {
    ...
    var servicesT = glob.sync(path.join(servicesPath, '**', '*.ts'));
    ...
    _.each(servicesT, function(serviceTFilePath) {
      var serviceName = getServiceNameFromFilePath(serviceTFilePath);

      serviceLibrary[serviceName] = {
        path: serviceTFilePath,
        directDependencies: getServiceDependencies(serviceTFilePath, 'service')
      };
    });
  ...
  var getBehaviorScripts = function (behaviorName) {
    ...
    var allScripts      = glob.sync(path.join(behaviorsPath, '**', '*.js'));
    var behaviorScript  = _.find(allScripts, function(script) {
      return behaviorName === script.replace(/^.*\/(.+)\.js$/,'$1').split('.').pop();
    });

    if (!behaviorScript) {
      var allTScripts = glob.sync(path.join(behaviorsPath, '**', '*.ts'));
      behaviorScript = _.find(allTScripts, function(script) {
        return behaviorName === script.replace(/^.*\/(.+)\.ts$/,'$1').split('.').pop();
      });
    }
  ...
  var getCoreIncludeDictionary = function() {
    ...
    var possibleDeps = _.union(
      ...
      glob.sync(path.join(servicesPath,   '**', '*.ts')),
      glob.sync(path.join(componentsPath, '**', '*.ts')),
      glob.sync(path.join(behaviorsPath,  '**', '*.ts')),
      glob.sync(path.join(guidesPath,     '**', '*.ts'))
    );
