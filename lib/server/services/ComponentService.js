...
var fs      = require("fs"),
    ...
    RevisionedFileUtil  = require('../utils/RevisionedFileUtil'),
    _     = require('lodash');

//Add String functions to _
_.str = require('underscore.string');

module.exports = function(wilsonConfig, logger, CacheService, BundleService) {
  ...
  var generateCoreBundle = function (ignoreCache) {
    ...
    BundleService.bundleExists(coreBundle, function (exists) {
      if (exists && !ignoreCache) {
        ...
        var coreIncludeDictionary = ConfigService.getCoreIncludeDictionary(config);
  ...
  var getServableComponent = function (componentName, filters) {
    ...
    if (componentLibrary[componentName]) {
      ...
      if (!(readFromCache && componentJson)) {
        ...
        //TODO: Make '/wilson' based on router.mountpoint
...
module.exports.$inject = ['wilson.config', 'Logger', 'CacheService', 'BundleService'];
