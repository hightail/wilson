...
module.exports = function(wilsonConfig, wilsonFrameworkConfig, logger, CacheService, BundleService, RevisionedFileUtil) {
  ...
  var getServiceNameFromFilePath = function (serviceFilePath) {
    var serviceName = serviceFilePath.split('/').pop().replace('.js', '');
  ...
  function findAllComponents() {
    ...
    console.log('componentScripts', componentScripts)
  ...
  var getCoreIncludeDictionary = function() {
    ...
    componentDeps = _.map(componentDeps, function(dep) { return dep.replace(/^.*\/([^\/]+).js$/, '$1'); });
    console.log('componentDeps', componentDeps);
    ...
    console.log('possibleDeps', possibleDeps);
    var depLib = {};
    ...
    _.each(possibleDeps, function(dep) {
      ...
      console.log('depLib[formalName]', { file: fileName, name: formalName, path: dep.replace(pathPrefix, '').replace(('/' + fileName), '') });
    });
