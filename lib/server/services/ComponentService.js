...
module.exports = function(wilsonConfig, wilsonFrameworkConfig, logger, CacheService, BundleService, RevisionedFileUtil) {
  ...
  var getIgnoredServices = function () {
    ...
    //var frameworkServices = fs.readdirSync(path.join(servicesPath, config.services.frameworkFolder));

//    _.each(frameworkServices, function (service) {
//      var segments = service.split('.');
//      while (segments.length && segments.length > 2) {
//        segments.shift();
//      }
//
//      ignoredServices.push(segments[0]);
//    });
  ...
  /**
   * Returns a list of all 'File' specific paths under a directory.
   *
   * @private
   * @method getFilePaths
   * @param relativePath
   ...
   * @returns {Array}
   */
  var getFilePaths = function (relativePath) {
    var pathList = [];

    if (fs.existsSync(relativePath)) {
      var services = fs.readdirSync(relativePath);

      // Push all base services files to the list and return them
      _.each(services, function (service) {
        var sPath = path.join(relativePath, service);
        var info = fs.lstatSync(sPath);
        if (info.isFile()) {
          pathList.push(sPath);
        }
      });
    } else {
      logger.warn(relativePath + " does not exist.");
    }
  ...
  var generateServiceLibrary = function () {
    ...
    var services = _.union(
      getFilePaths(servicesPath),   // Base Services
      getFilePaths(parsersPath),    // Parsers,
      getFilePaths(utilitiesPath),  // Utilities,
      getFilePaths(resourcesPath)   // Resources
    );
  ...
  function getResourceType(resourceName) {
    //TODO: This needs to handle all types (e.g. components, guides, etc)
    var types = ['services', 'utilities', 'parsers', 'resources'];
    var typeMatchFound = false;

    for (var typeIndex = 0; !typeMatchFound && typeIndex < types.length; typeIndex++) {
      var type = types[typeIndex];
      var selectorsForType = wilsonConfig.server.dependencies.selectors[type];

      for (var selectorIndex = 0; selectorIndex < selectorsForType.length; selectorIndex++) {
        var selector = selectorsForType[selectorIndex];

        if(_.str.endsWith(resourceName, selector)) {
          console.log('resourceName: ' + resourceName + ' type:' + type);
          typeMatchFound = true;
          break;
        }
      }
  ...
  var getCoreIncludeDictionary = function() {
    ...
    var coreDeps = _.difference(wilsonConfig.server.dependencies.core, ignoredDeps);

    _.each(coreDeps, function(resourceName) {
      var resourceType = getResourceType(resourceName);
      if (resourceType) {
        var includePath = paths[resourceType];

        if (resourceType === 'behaviors') {
          includePath = path.join(resourceConfig.path, resourceName);
        }

        var curIncludes = coreIncludes[includePath];
          if (!curIncludes) {
            curIncludes = coreIncludes[includePath] = [];
          }
          curIncludes.push(resourceName);
      } else {
        logger.error('Unable to determine type of core dependency: ' + resourceName);
      }
    });
