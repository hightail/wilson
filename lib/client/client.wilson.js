...
(function(window, angular, _) {
   ...
   */
  var wilsonModule = angular.module('wilson', ['ngRoute', 'LocalStorageModule', 'wilson.config', 'wilson.i18n'])
    .config(['$interpolateProvider', '$locationProvider', 'i18nextServiceProvider', '$routeProvider', 'localStorageServiceProvider', '$compileProvider', '$controllerProvider', '$provide',
      ...
      // If we are running in iframe mode, the setup our interface for postMessage
      //TODO: Uncomment this after KL implements VRI/upgrade flows
      //TODO: See CAS-2066 for more info
//      if (angular.wilson.config.appOptions.iframe) {
//
//        // If we are not actually running in a frame, we don't allow this shiz
//        // time to evacuate all content :) he he he....
//        if ('production' === angular.wilson.config.deploy.mode && window.self === window.top) {
//          document.body.innerHTML = '<!-- Unsafe Access, Content Aborted -->';
...
//      }
  ...
  var getRouteOptions = function(path) {
    ...
    return (routeConfig && routeConfig.options) ? routeConfig.options : { };
  };
...
//              $(angular.wilson.config.app.appLoaderSelector || '.loading-screen').css('display', 'block');
  ...
  // legacy stuff during phi -> wilson migration
  var getPhiUrl = function(path) {
    if (angular.wilson.config.legacy) {
      path = path || '';

      var lng = angular.wilson.config.i18n.lng;
      var host = angular.wilson.config.legacy.phiHost;

      if (lng && !_.contains(['en', 'en-US'], lng)) {
        host = host.replace(/^[a-z0-9]+\./i, lng + '.');
      }
  ...
  angular.wilson = window.HtFramework = wilson;
})(this, angular, _);
;/**
...
angular.wilson.service('ComponentFactoryService',
  ['StateMachineService', 'BusFactoryService', 'i18nextService', 'AppStateService', 'TranslationOverrideService',
  function(StateMachineService, BusFactoryService, i18nextService, AppStateService, TranslationOverrideService) {
     ...
     */
    var init = function(componentName, controller, $scope, $attrs) {
      /**
      ...
      $scope.componentCName = componentName;
...
//      });
       ...
       * @param handler
       */
      controller.autosubscribe = function(IEventTarget, eventName, handler) {
       ...
       * @param handler
       */
      controller.autoadd = function(signal, handler) {
    ...
    var addElementSupport = function(controller, $scope, $element) {
       ...
       */
      controller.autoOn = function(eventName, handler, elementToListenOn) {
       ...
       */
      controller.autoApplyOn = function(eventName, handler, handlerContext, elementToListenOn) {
        //default elementToListenOn to $element
        if (!elementToListenOn) {
          elementToListenOn = $element;
        }
