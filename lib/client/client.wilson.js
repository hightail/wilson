...
(function(window, angular, _) {
  ...
  function Wilson(module) {
    ...
    function createPageLevelController(name, originalController, scriptDependencies) {
      return ['ComponentFactoryService', 'ResourceLoaderService', 'IRouteService', 'WilsonUtils', '$injector', '$scope',
        function(ComponentFactoryService, ResourceLoaderService, IRouteService, WilsonUtils, $injector, $scope) {
          var controller  = this;
          ...
          if (_.isArray(scriptDependencies) && !_.isEmpty(scriptDependencies)) {
            ResourceLoaderService.loadResourceScripts(scriptDependencies).then(
              function() { if (_.isFunction(controller.onReady))  { controller.onReady.apply(controller, []); } },
              function() { if (_.isFunction(controller.onFailed)) { controller.onFail.apply(controller, []);  } }
            );
          }
          ...
          // Set this page-level component as our active page
          _activePage = name;
        }
      ];
    }
    ...
    function createRoute(routeInfo, routes, routeProvider) {
      ...
      var routingData = {
        ...
        resolve: {
          ...
          $template:  ['$rootScope', '$route', '$q', '$location', '$window', '$templateCache', 'AppStateService', 'IRouteService', 'ComponentLoaderService',
            function($rootScope, $route, $q, $location, $window, $templateCache, AppStateService, IRouteService, ComponentLoaderService) {
              ...
              return IRouteService.handleRouteChange(currentRoute, options, routeInfo).then(function() {
                ...
                return ComponentLoaderService.load(routeInfo.component).catch(function(error) {
     ...
     */
    this.getActiveComponentDetails = function getActiveComponentDetails() {
      return JSON.stringify(_.map(_componentMap, function(component, id) {
        return _.padEnd(component.name, 34, ' ') + '::' + id;
      }), null, 2);
    };
    ...
    _module.config(['$interpolateProvider', '$locationProvider', 'i18nextServiceProvider', '$routeProvider', 'localStorageServiceProvider', '$compileProvider', '$controllerProvider', '$filterProvider', '$provide',
      function wilsonConfig($interpolateProvider, $locationProvider, i18nextServiceProvider, $routeProvider, localStorageServiceProvider, $compileProvider, $controllerProvider, $filterProvider, $provide) {
        ...
        _compileProvider.debugInfoEnabled(!config.disableDebugInfo);
    ...
    // Angular Module Config Routine
    _module.run(['$rootScope', '$templateCache', '$location', '$timeout', 'WilsonUtils',
      function wilsonRun($rootScope, $templateCache, $location, $timeout, WilsonUtils) {

        // Local No Operation Function
        var noop = function() {};

        // Decorate utilities onto this wilson instance
   ...
   */
  var wilsonModule  = angular.module('wilson', ['ngRoute', 'LocalStorageModule', 'wilson.config', 'wilson.i18n', 'wilson.utils', 'wilson.decorators']);
...
angular.module('wilson.utils', []).provider('WilsonUtils', function() {
  ...
  //   _____                       _   _ _   _ _
  //  | ____|_ __ _ __ ___  _ __  | | | | |_(_) |___
  //  |  _| | '__| '__/ _ \| '__| | | | | __| | / __|
  //  | |___| |  | | | (_) | |    | |_| | |_| | \__ \
  //  |_____|_|  |_|  \___/|_|     \___/ \__|_|_|___/
  //
  //
  // region error utils
...
angular.wilson.service('ComponentFactoryService',
  ['$rootScope', '$timeout', 'StateMachineService', 'i18nextService', 'AppStateService', 'TranslationOverrideService',
  function($rootScope, $timeout, StateMachineService, i18nextService, AppStateService, TranslationOverrideService) {
