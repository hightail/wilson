  ...
  function Wilson(module) {
    ...
    function createComponentController(name, controller, config) {
      return ['ComponentFactoryService', 'ResourceLoaderService', 'WilsonUtils', '$injector', '$scope', '$element', '$attrs',
        function(ComponentFactoryService, ResourceLoaderService, WilsonUtils, $injector, $scope, $element, $attrs) {
          ...
          ComponentFactoryService.create(componentId, name, controller, this, parent, $scope, $element, $attrs, config);

          // Register this component scope with wilson
          _componentMap[componentId] = $scope;
    ...
    this.component = function defineComponent(name, config) {
      ...
      if (!_cache.components[directiveName]) {
        ...
        // Transform direct config properties to support short-hand
        if (config.bindings) { config.bindToController = config.bindings; }   // Support bindings short-hand
        ...
        var fullConfig = {
          ...
          replace:        true,
          scope:          {},
          controllerAs:   _appConfig.defaultCtrlAs || false
        };
        ...
        // Forward self as required controller if other requirements are specified
        if (_.isArray(fullConfig.require)) { fullConfig.require.unshift(directiveName); }
...
angular.wilson.service('ComponentFactoryService', ['$rootScope', '$timeout', '$injector', 'WilsonComponent',
  function($rootScope, $timeout, $injector, WilsonComponent) {
    ...
    function createComponent(componentId, componentName, controller, context, parentScope, $scope, $element, $attrs, config) {
      ...
      $scope.state          = { current: 'NA' };  // Default state
      ...
      // Add convenience method for root broadcasting
      $scope.$broadcastRoot = $rootScope.$broadcast;
          ...
          _.forIn(config.exports, function(construct, property) {
            if (_.isFunction($scope[construct])) {
              ...
              Object.defineProperty(exports, property, {
                ...
                set: function(val)  { $scope[construct] = val; $scope.triggerDigest(); }
              });
            }
          });
...
angular.wilson.service('ComponentLoaderService', ['ResourceLoaderService', '$q', '$http',
