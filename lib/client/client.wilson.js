...
(function(window, angular, _) {
  ...
  var validateParams = function(type, name, definition, link) {
    ...
    var validateClassName = function(type, name) {
      var suffix = _.classify(type);
      if (type === 'service') {
        suffix = '[Service|Factory|Utility|Resource]';
      }
      var formalName = _.classify(name);
...
//              $(angular.wilson.config.app.appLoaderSelector || '.loading-screen').css('display', 'block');
          ...
          dependencies: ['$q', 'IRouteService', function($q, IRouteService) {
            var loadDependencies = $q.when();
            var loadSession      = $q.when();

            // Call load dependencies on IRouteService if exposed (this method should return a $q promise)
            if (IRouteService.loadDependencies) {
              loadDependencies = IRouteService.loadDependencies(routeInfo);
            }

            if (IRouteService.loadSession) {
              loadSession = IRouteService.loadSession();
            }

            return $q.all([loadDependencies, loadSession]);
          }],
  ...
  var defineGuide = function(name, definition) {
    ...
    if (!_cache.guides[name]) {
      ...
      provider.directive(_.camelCase(name), ['i18nextService', function(i18nextService) {
        return {
          restrict: 'A',
      ...
      $scope.translate = controller.translate = function(text, options) {
        ...
        if (!ignoredTextOverrides[text]) {
          ...
          if (textOverrides[text]) {
            ...
            //console.log(_.sprintf('override exists! ns: %s  value:%s', options.ns, i18nextService.translate(text, options)));
...
//      });
      ...
      // Closured var for our digest cancel
       ...
       * @param handle
       */
      var autoAfterDigest = function autoAfterDigest(handle) {
  ...
  ['ResourceLoaderService', '$q', '$http', '$compile', '$rootScope', function(ResourceLoaderService, $q, $http, $compile, $rootScope) {
    ...
    var getComponent = function(componentName) {
      ...
      if (connectionFilters) {
        componentPath += _.sprintf('/%s', connectionFilters);
      }
...
(function(window, _) {
    ...
    function addTimeoutEventListeners(fsm, cfg) {
        ...
        fsm.on('onenterstate', function(event) {
            var timeout = _.find(timeouts, {state: event.to});
            if (timeout) {
