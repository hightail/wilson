        ...
        resolve: {
          ...
          component:  ['$route', '$q', '$location', '$window', 'WilsonRouter', 'WilsonUtils', 'ComponentLoaderService',
            function($route, $q, $location, $window, WilsonRouter, WilsonUtils, ComponentLoaderService) {
              ...
              console.log("routingData: printing1 currentRoute: ", currentRoute);
              debugger;
              ...
              return WilsonRouter.handleRouteChange(currentRoute, options, routeInfo).then(function() {
                ...
                console.log("routingData: printing2 routeInfo", routeInfo);
                return ComponentLoaderService.load(routeInfo.component);
                console.log("routingData: printing3");
              }).then(function(data) {
                // Force a reload to update if out of date component
                console.log("routingData: printing4");
                if (data.version !== angular.wilson.config.app.version) { $window.location.reload(); }
                console.log("routingData: printing5");
                debugger;
                return data;
              }).catch( function(err) {console.log("routingData: printing error section: ", err); return $q.reject(err); });
            }
          ],
...
angular.module('wilson.logger', []).provider('WilsonLogger', function() {
  ...
  this.$get = [function() {
    return Object.freeze({
      ...
      debug: function() { console.log("logger;;;;;;;;;;;;;;;;;;");_logMethods.debug.apply(this, arguments);  },
...
wilson.service('ComponentLoaderService', ['ResourceLoaderService', '$q', '$http',
  function(ResourceLoaderService, $q, $http) {
    ...
    console.log("Inside ComponentLoaderService1: componentName: ", componentName);
      var cachedComponent  = _componentCache[componentName];

      // If component is not cached then fetch it and store its promise
      if (!cachedComponent) {
      console.log("Inside ComponentLoaderService2: componentName: ");
        ...
        return (cachedComponent.promise = fetchComponentData(componentName).then(function(compData) {
          ...
          console.log("Inside ComponentLoaderService3: componentName: ");
          if (_appVersion === compData.version) {
          console.log("Inside ComponentLoaderService4: componentName: ");
            return ResourceLoaderService.loadResourceBundle(compData).then(function() { console.log("Inside ComponentLoaderService5: returning data: ");  return compData; }, function(e) {
                                                                                                                                                                                      console.log("Inside ComponentLoaderService6: componentName: ");
...
console.log("Inside ComponentLoaderService4: componentName: ");
        ...
        console.log("Inside ComponentLoaderService5: componentName: ");
...
wilson.service('ResourceLoaderService', ['$q', '$templateCache', function($q, $templateCache) {
    ...
    console.log("Inside loadResourceBundle: loadScript", src);
      ...
      console.log("Inside loadResourceBundle: loadScript 2", src);
      var deferred  = $q.defer();
      console.log("Inside loadResourceBundle: loadScript 3", src);
      $.getScript(scriptUrl).done(function(script, textStatus) {
      console.log("Inside loadResourceBundle: loadScript SUCCESS wilson;log", wilson.log.debug);
      debugger;
//        wilson.log.debug('SUCCESS: ' + src + ' LOADED (' + textStatus + ')');
        _scriptCache[src] = true;  // Mark entry in cache for this script src
        deferred.resolve();
        console.log("Inside loadResourceBundle: loadScript SUCCESS", src);
      }).fail(function(jqxhr, settings, exception) {
      console.log("Inside loadResourceBundle: loadScript ERROR", src);
