  ...
  var wilsonModule = angular.module('wilson', ['ngRoute', 'LocalStorageModule', 'wilson.config', 'wilson.i18n'])
    .config(['$interpolateProvider', '$locationProvider', 'i18nextServiceProvider', '$routeProvider', 'localStorageServiceProvider', '$compileProvider', '$controllerProvider', '$provide',
      function($interpolateProvider, $locationProvider, i18nextServiceProvider, $routeProvider, localStorageServiceProvider, $compileProvider, $controllerProvider, $provide) {
        ...
        //Override ngRepeat to allow for prerendering
        _compileProvider.directive('ngRepeat', ['$timeout', function($timeout) {
...
angular.wilson.service('ComponentFactoryService',
  ['StateMachineService', 'BusFactoryService', 'i18nextService', 'AppStateService', 'TranslationOverrideService', '$timeout',
  function(StateMachineService, BusFactoryService, i18nextService, AppStateService, TranslationOverrideService, $timeout) {
    ...
    var init = function(componentName, $scope, $element, $attrs, controller) {
      //Prerender view dependency info
      var viewDepCount = 0;
...
//      });
      ...
      $scope.resolveViewDependency = controller.resolveViewDependency = function(count) {
        ...
        if (viewDepCount === 0) {
          console.log('All view dependencies have resolved for component ' + componentName);
      ...
      $scope.checkViewDependencies = controller.checkViewDependencies = function() {
        $timeout(function() {
          $timeout(function() {
            //console.log(_.str.sprintf('Checking deps for [%s](1)', componentName));
            if (viewDepCount === 0  && $scope.parentComponent && $scope.parentComponent.componentCName) {
              //console.log(_.str.sprintf('All deps resolved for [%s](1)', componentName));
              $scope.parentComponent.resolveViewDependency();
            }
          }, 0);
        }, 0);
      ...
      controller.registerDataDependency = function(key, validationFunc) {
        console.log(_.str.sprintf('Registering data dep: %s.%s', componentName, key));
        ...
        var removeWatch = $scope.$watch(key, function(newValue) {
          if (validationFunc(newValue)) {
            removeWatch();
            console.log(_.str.sprintf('Resolved data dep: %s.%s', componentName, key));
